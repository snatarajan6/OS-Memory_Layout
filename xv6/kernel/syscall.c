...
{
  if(((addr < (USERTOP - (p->proc_stack_sz*PGSIZE))) &&(addr >= p->sz)) || ((addr+4 < (USERTOP - (p->proc_stack_sz*PGSIZE))) &&(addr+4 > p->sz)) || (addr >= USERTOP) || (addr+4 > USERTOP) || (addr == 0 )) 
    return -1;
...
argptr(int n, char **pp, int size)
{
  ...
  if((((uint)i < (USERTOP - (proc->proc_stack_sz*PGSIZE))) &&((uint)i >= proc->sz)) || (((uint)(i+size) < (USERTOP - (proc->proc_stack_sz*PGSIZE))) &&((uint)(i+size) > proc->sz)) || ((uint)i >= USERTOP) || ((uint)(i+size) > USERTOP) || ((uint)i == 0 )) 
    return -1;
